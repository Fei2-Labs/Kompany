...
| Anchor Kit + Brand Lock (§10) | `workflows/brand_lock.*`: requires approved direction + ≥1 active asset → CV anchor audit (provenance-only; no pixel vision yet) → machine-readable Brand Lock (exact fonts verified, color tokens/roles, logo rules, imagery, graphic language, composition, whitespace, hierarchy, materials, packaging rules, prohibited styles, approved anchor ids) → `brand_lock_change` card. **Simplification:** the `anchor_brand_kit` approval rides on the same card (`approved_anchor_ids`); missing anchor or colors/typography = blocker → `revise`. Approving lock vN+1 supersedes vN and marks lock-dependent assets stale (impact previewed on the card). |
...
Remaining deltas vs the handoff (not blockers): a separate `anchor_brand_kit` card (folded onto `brand_lock_change`); pixel-level CV audit (provenance-only today); additional providers (Lovart / Canva) behind the same `VisualProvider` ABC; CISO/CoS steps inside `brand-foundation`; Core resume-after-approval for YAML steps declared `autonomy_tier: approval`; other `ToolContext` construction sites in Core still pass 1.0.0 fields only (defaults keep them working).
