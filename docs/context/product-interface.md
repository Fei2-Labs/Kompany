...
**Meaning:** A Telegram bot is another interface in the same adapter pattern as CLI/API/MCP/SDK. It connects to the running engine and supports the three user interaction modes: directive (issue commands), query (check status/spending/progress), and override (intervene with risk assessment). AutonomyGate approval requests can be pushed as Telegram messages, and the user can approve/reject directly from their phone. Other messaging platforms (Discord, Slack, WeChat) can be added as additional adapters later.

**Implication:** Remote access does not change the engine or decision chain — it is purely an interface-layer addition. Security is critical: Telegram bot must authenticate the user and encrypt sensitive data.
